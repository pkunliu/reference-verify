...
- Export arXiv-only papers with canonical `journal = {arXiv preprint arXiv:ID}`, matching `eprint`, and `archivePrefix = {arXiv}` fields; omit those arXiv fields from formally published records.
